...
raw_data <- read_csv("gss_social_identity.csv")
dict <- read_lines("gss_dict_2013.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("gss_labels_2013.txt")
...
# Fix the names
gss2013_selected <- gss2013_selected %>% 
  rename(age = agegr10,
         sex = sex,
         marital_status = marstat,
         grouped_education = dh1ged,
         likely_to_vote = vbr_25,
         interest_in_politics = rep_05) 
...
gss2013_selected <- gss2013_selected %>% 
  mutate(age = case_when(
    age == 1 ~ "15 to 24 years",
    age == 2 ~ "25 to 34 years",
    age == 3 ~ "35 to 44 years",
    age == 4 ~ "45 to 54 years",
    age == 5 ~ "55 to 64 years", 
    age == 6 ~ "65 to 74 years",
...
gss2013_selected <- gss2013_selected %>% 
  mutate(grouped_education = case_when(
    grouped_education == 1 ~ "Less than High School",
    grouped_education == 2 ~ "Graduated from High School",
    grouped_education == 3 ~ "Post-secondary diploma",
    grouped_education == 4 ~ "University degree"
    ))
...
gss2013_selected <- gss2013_selected %>% 
  mutate(likely_to_vote = case_when(
    likely_to_vote == 1 ~ 1,
    likely_to_vote == 2 ~ 2,
    likely_to_vote >= 3 & likely_to_vote <= 4 ~ 3,
  ))

gss2013_selected <- gss2013_selected %>% 
  mutate(interest_in_politics = case_when(
    interest_in_politics == 1 ~ "Very interested",
    interest_in_politics == 2 ~ "Somewhat interested",
    interest_in_politics == 3 ~ "Not very interested",
    interest_in_politics == 4 ~ "Not at all interested"
  ))

gss2013_selected <- na.omit(gss2013_selected)


gss2013_selected <- 
  gss2013_selected %>%
  count(age, sex, marital_status, grouped_education, likely_to_vote, interest_in_politics) %>%
  group_by(age, sex, marital_status, grouped_education, likely_to_vote, interest_in_politics)

write_csv(gss2013_selected, "gss2013_census.csv")
